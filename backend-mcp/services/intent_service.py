...
def create_intent(author : str, content : str):
    ...
    report = create_intent_report(intent_id, f"Intent {intent_id} created by {intent.get("author")} with content: {content} at {creation_date}")

    handler.handle_intent(intent, create_intent_report)
...
def create_intent_report(intent_id : str, content : str):
    ...
    response = repository.insert_report_into_graphdb(report_id_response, intent_id, "no name", result, (time.time() - startTime), creation_date)
